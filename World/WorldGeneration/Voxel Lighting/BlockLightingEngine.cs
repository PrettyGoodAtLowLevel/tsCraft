...
namespace OurCraft.World.WorldGeneration.Voxel_Lighting
{
    ...
    {
        const int CHUNK_SIZE = WorldConstants.CHUNK_WIDTH; //32
        const int SUBCHUNK_SIZE = WorldConstants.SUBCHUNK_SIZE_IN_BLOCKS;
        const int MAX_HEIGHT = WorldConstants.CHUNK_HEIGHT;

        const int MIN_LIGHT = LightConstants.MIN_LIGHT;
        const int LOW_LIGHT = LightConstants.MIN_LIGHT + 1;
        ...
        {
            if (chunk == null || !chunk.HasAllBlocks() || chunk.Deleted()) return;

            for (int y = 0; y < Chunk.HEIGHT_IN_SUBCHUNKS; y++)
            {
                SubChunk subChunk = chunk.SubChunks[y];
                foreach (var lightSource in subChunk.lightSources)
                {
                    ...
                    int globalX = lx + (CHUNK_SIZE * chunk.ChunkPos.X);
                    int globalY = ly + (SUBCHUNK_SIZE * subChunk.ChunkYPos);
                    int globalZ = lz + (CHUNK_SIZE * chunk.ChunkPos.Z);

                    chunk.SetBlockLight(lx, globalY, lz, lightValue);
        ...
        {
            if (chunk == null || !chunk.HasAllBlocks() || chunk.Deleted())
                return;
