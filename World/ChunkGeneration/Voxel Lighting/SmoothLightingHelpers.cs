...
namespace OurCraft.Graphics.Voxel_Lighting
{
    ...
    public static class SmoothLightingHelpers
    {
        ...
        public static ushort SampleVertexLight(int x, int y, int z, int offsetX, int offsetY, int offsetZ, CubeFaces face, int vertexIndex, ChunkSectionNeighbors nc, bool sampleInner)
        {
            ...
            {
                ushort l0 = sampleInner ? ChunkBuilder.GetLightSafe(x, y, z, 0, 0, 0, nc) //sample inner if face is inner
                : ChunkBuilder.GetLightSafe(x, y, z, offsetX + (int)s0.X, offsetY + (int)s0.Y, offsetZ + (int)s0.Z, nc);
                ushort l1 = ChunkBuilder.GetLightSafe(x, y, z, offsetX + (int)s1.X, offsetY + (int)s1.Y, offsetZ + (int)s1.Z, nc);
                ushort l2 = ChunkBuilder.GetLightSafe(x, y, z, offsetX + (int)s2.X, offsetY + (int)s2.Y, offsetZ + (int)s2.Z, nc);
                ushort l3 = ChunkBuilder.GetLightSafe(x, y, z, offsetX + (int)s3.X, offsetY + (int)s3.Y, offsetZ + (int)s3.Z, nc);

                return AveragePackedLight(l0, l1, l2, l3);
            }
            else
            {
                ushort l0 = sampleInner ? ChunkBuilder.GetLightSafe(x, y, z, 0, 0, 0, nc)
                : ChunkBuilder.GetLightSafe(x, y, z, offsetX + (int)s0.X, offsetY + (int)s0.Y, offsetZ + (int)s0.Z, nc);
                ushort l1 = ChunkBuilder.GetLightSafe(x, y, z, offsetX + (int)s1.X, offsetY + (int)s1.Y, offsetZ + (int)s1.Z, nc);
                ushort l2 = ChunkBuilder.GetLightSafe(x, y, z, offsetX + (int)s2.X, offsetY + (int)s2.Y, offsetZ + (int)s2.Z, nc);
        ...
        public static byte SampleAO(int x, int y, int z, int offsetX, int offsetY, int offsetZ, CubeFaces face, int vertexIndex, ChunkSectionNeighbors nc)
        {
            ...
            if (ChunkBuilder.IsNeighborSolidSafe(x, y, z, offsetX + (int)s1.X, offsetY + (int)s1.Y, offsetZ + (int)s1.Z, nc)) solid++;
            if (ChunkBuilder.IsNeighborSolidSafe(x, y, z, offsetX + (int)s2.X, offsetY + (int)s2.Y, offsetZ + (int)s2.Z, nc)) solid++;
            if (ChunkBuilder.IsNeighborSolidSafe(x, y, z, offsetX + (int)s3.X, offsetY + (int)s3.Y, offsetZ + (int)s3.Z, nc)) solid++;
