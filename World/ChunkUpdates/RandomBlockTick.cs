...
namespace OurCraft.World.ChunkUpdates
{
    public static class RandomBlockTick
    {
        ...
        private static void TickSubChunk(SubChunk subChunk, ChunkManager world)
        {
            ...
            for (int i = 0; i < WorldConstants.DEFAULT_RANDOM_TICK; i++)
            {
                ...
                int wx = (chunk.ChunkPos.X * WorldConstants.CHUNK_WIDTH) + (subChunk.ChunkXPos * WorldConstants.SUBCHUNK_SIZE_IN_BLOCKS) + localX;
                int wy = (subChunk.ChunkYPos * WorldConstants.SUBCHUNK_SIZE_IN_BLOCKS) + localY;
                int wz = (chunk.ChunkPos.Z * WorldConstants.CHUNK_WIDTH) + (subChunk.ChunkZPos * WorldConstants.SUBCHUNK_SIZE_IN_BLOCKS) + localZ;
