...
namespace OurCraft.Blocks.Meshing
{
    public class BlockLogShape : BlockShape
    {
        ...
        public override void AddBlockMesh(Vector3 pos, BlockState bottom, BlockState top, BlockState front, BlockState back, BlockState right, BlockState left, ChunkMeshData mesh, BlockState thisState, VoxelAOData aOData)
        {
            ...
            RightFaceTex, LeftFaceTex, FrontFaceTex, BackFaceTex, TopFaceTex, TopFaceTex, mesh, aOData);

            else if (axis == Axis.Z) BlockMeshBuilder.BuildFullBlock(pos, bottom, top, front, back, right, left, thisState,
            RightFaceTex, LeftFaceTex, TopFaceTex, TopFaceTex, FrontFaceTex, BackFaceTex, mesh, aOData);

            else if (axis == Axis.Y) BlockMeshBuilder.BuildFullBlock(pos, bottom, top, front, back, right, left, thisState,
            TopFaceTex, TopFaceTex, FrontFaceTex, BackFaceTex, RightFaceTex, LeftFaceTex, mesh, aOData);
        }
