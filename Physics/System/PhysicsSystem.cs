...
namespace OurCraft.Physics.System
{
    ...
    public class PhysicsSystem : BaseSystem<PhysicsObj>
    {
        ...
        {
            AABB rbBox = AABBHelpers.GetAABB(pos, boundsMin, boundsMax);
            ...
            for (int z = minZ; z <= maxZ; z++)
            {
                 ...
                 foreach (var aabb in colShape.aabbs)
                 {
                    ...
                    if (!block.IsPhysicsSolid)
                    {
                        ...
                        if (blockPhysics.isFluid && AABBHelpers.IntersectsLocal(aabb, rbBox, new Vector3d(x, y, z), Vector3d.Zero))
                        if (blockPhysics.isFluid && AABBHelpers.IntersectsLocal(aabb, rbBox, new Vector3d(x, y, z), Vector3d.Zero))
                        {
                            if (AABBHelpers.PointIntersectsLocal(body.HeadPosition, aabb, new Vector3d(x, y, z))) body.underWater = true;
                            body.inFluid = true;
                            continue;
                        }
                        continue;
                    ...
                    allowed = AABBHelpers.ClipAxisLocal(rbBox, aabb, new Vector3d(x, y, z), move.X, CollisionAxis.X, allowed);
                    allowed = AABBHelpers.ClipAxisLocal(rbBox, aabb, new Vector3d(x, y, z), move.Y, CollisionAxis.Y, allowed);
                    allowed = AABBHelpers.ClipAxisLocal(rbBox, aabb, new Vector3d(x, y, z), move.Z, CollisionAxis.Z, allowed);
                 }
            }
        ...
        static void ResolveAxisWithStep(ChunkManager world, PhysicsObj body, ref Vector3d pos, ref Vector3d vel, ref Vector3d move, bool isX)
        {
            ...
            if (body.sneaking && body.grounded && !body.inFluid && resolved != 0)
            {
                ...
                AABB testBox = AABBHelpers.GetAABB(testPos, body.boundsMin, body.boundsMax);
