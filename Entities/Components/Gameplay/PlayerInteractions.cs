...
namespace OurCraft.Entities.Components.Gameplay
{
    ...
    public class PlayerInteractions : Component
    {
        ...
        {
            bool hitBlock = PhysicsHelpers.RaycastVoxel(orientation.WorldPosition, orientation.Forward, reach,
            ...
            if (ms.IsButtonPressed(MouseButton.Left))
            {
                if (hitBlock)
                {
                    BlockState old = world.GetBlockState(hit.blockPos);
                    old.OnRemove(hit.blockPos, world);
                    world.SetBlock(hit.blockPos, Block.AIR);

                    UpdateNeighborBlocks(hit.blockPos, Vector3i.Zero, world);
                }
            }
            if (ms.IsButtonPressed(MouseButton.Right))
            {
                if (hitBlock)
                {
                    ...
                    if (AABBHelpers.IntersectsLocal(aabb, AABBHelpers.GetAABB(rb.position, rb.boundsMin, rb.boundsMax), hit.blockPos + hit.faceNormal, Vector3d.Zero)) return;

                    //try to add block
                    BlockRegistry.GetBlock(currentBlockID).PlaceBlockState(hit.blockPos, hit.faceNormal,
                    world.GetBlockState(hit.blockPos), world);
                    world.GetBlockState(hit.blockPos + hit.faceNormal).OnPlaced(hit.blockPos + hit.faceNormal, world);

                    UpdateNeighborBlocks(hit.blockPos, hit.faceNormal, world);
                }
            }
        ...
        void UpdateNeighborBlocks(Vector3i blockPos, Vector3i faceNormal, ChunkManager world)
        {
            BlockState bottom, top,  front, back, left, right;

            bottom = world.GetBlockState(blockPos + faceNormal - Vector3i.UnitY);
            top = world.GetBlockState(blockPos + faceNormal + Vector3i.UnitY);
            front = world.GetBlockState(blockPos + faceNormal + Vector3i.UnitZ);
            back = world.GetBlockState(blockPos + faceNormal - Vector3i.UnitZ);
            left = world.GetBlockState(blockPos + faceNormal - Vector3i.UnitX);
            right = world.GetBlockState(blockPos + faceNormal + Vector3i.UnitX);

            bottom.OnNeighborChanged(blockPos + faceNormal - Vector3i.UnitY, world);
            top.OnNeighborChanged(blockPos + faceNormal + Vector3i.UnitY, world);
            front.OnNeighborChanged(blockPos + faceNormal + Vector3i.UnitZ, world);
            back.OnNeighborChanged(blockPos + faceNormal - Vector3i.UnitZ, world);
            left.OnNeighborChanged(blockPos + faceNormal - Vector3i.UnitX, world);
            right.OnNeighborChanged(blockPos + faceNormal + Vector3i.UnitX, world);
        }
        ...
        static void SpawnTestEntity(KeyboardState ks, Transform orientation)
        {
            if (ks.IsKeyPressed(Keys.Y))
            {
                ...
                if (rand % 12 == 0) model.LoadModel("EntityModels/horse.json", "Textures/Mc Skins/horseBlack.png");
                else if (rand % 12 == 1) model.LoadModel("EntityModels/cloveModel.json", "Textures/Mc Skins/Clove val mc skin.png");
                else if (rand % 12 == 2) model.LoadModel("EntityModels/creeper.json", "Textures/Mc Skins/creeper.png");
                else if (rand % 12 == 3) model.LoadModel("EntityModels/ravager.json", "Textures/Mc Skins/ravager.png");
                else if (rand % 12 == 4) model.LoadModel("EntityModels/enderMan.json", "Textures/Mc Skins/enderman.png");
                else if (rand % 12 == 5) model.LoadModel("EntityModels/witch.json", "Textures/Mc Skins/witch.png");
                else if (rand % 12 == 6) model.LoadModel("EntityModels/wolf.json", "Textures/Mc Skins/wolf.png");
                else if (rand % 12 == 7) model.LoadModel("EntityModels/hoglin.json", "Textures/Mc Skins/hoglin.png");
                else if (rand % 12 == 8) model.LoadModel("EntityModels/panda.json", "Textures/Mc Skins/panda.png");
                else if (rand % 12 == 9) model.LoadModel("EntityModels/evoker.json", "Textures/Mc Skins/evoker.png");
                else if (rand % 12 == 10) model.LoadModel("EntityModels/warden.json", "Textures/Mc Skins/warden.png");
                else model.LoadModel("EntityModels/spider.json", "Textures/Mc Skins/spider.png");
        ...
        static void DebugChunkState(ChunkManager world, KeyboardState ks)
        {
            if (ks.IsKeyPressed(Keys.G))
            {
                ...
                + " Chunk Currently Generating? : " + chunk.generating);                
                world.Debug();
            }
        }
        ...
        static void DebugProfiling()
        {
            ...
            //Console.WriteLine("Current FPS: " + (int)(1 / Time.UnscaledDeltaTime));
